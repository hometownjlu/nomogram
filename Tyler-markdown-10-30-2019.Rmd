...
5. `all_data$Gender` - PArticipating in the couples match? 2 level categorical
...
`all_data` is a dataframe of the independent and the dependent variables for review. Each variable is contained in a column, and each row represents a single unique medical student. All variables now have appropriate data types.
...
```{r DataExplorer, results='asis', echo=FALSE, include=TRUE, align = 'left', cache=TRUE, out.width="75%"}
...
table_train_data <- arsenal::tableby(~.,
                                    data=train,
                                    control = tableby.control(test = TRUE,
                                                              total = F,
                                                              digits = 1L,
...
summary(table_train_data,
        text=T,
        title = 'Table: Applicant Descriptive Variables in the train dataframe from 2015 to 2018',
        labelTranslations = mylabels, #Seen in additional functions file
        pfootnote=TRUE)
```
...
```{r, include = TRUE, results="asis"}
table_test_data <- arsenal::tableby(~.,
                                    data=test,
                                    control = tableby.control(test = TRUE,
                                                              total = F,
                                                              digits = 1L,
...
summary(table_test_data,
        text=T,
        title = 'Table: Applicant Descriptive Variables in the test dataframe from 2015 to 2018',
        labelTranslations = mylabels, #Seen in additional functions file
        pfootnote=TRUE)
```
...
psych::pairs.panels(train[, c(9:15, 18)], bg=c("red","blue")[as.factor(train$Match_Status)], pch=21, jiggle = TRUE, scale = TRUE)
```
...
```{r, include=TRUE}
nomogram_kitchen_sink <- rms::nomogram(kitchen.sink,
                                                    ...
                                                    abbrev = F,
                                                    minlength = 9)
nomogram_image_kitchen_sink <- plot(nomogram_kitchen_sink, lplabel="Linear Predictor",
...
This `kitchen.sink` model accounts for just over `r kitchen.sink$stats[[10]]*100`% (r.squared) of the variation in sbp_diff in our training sample of `r nrow(train)` medical students in `train`.  
...
glance(lm.fit2)
```
This lm.fit2 model accounts for just over `r glance(lm.fit2)[[1]]*100`% (r.squared) of the variation in sbp_diff in our training sample of `r nrow(train)` medical students in `train`.  

The lm.fit2 adjusted R2 (`r glance(lm.fit2)[[2]]`) is very close to the raw R2 (`r glance(lm.fit2)[[1]]`), suggesting that we’re not likely to have a serious problem with collinearity.

The lm.fit2 ANOVA F test p.value (`r glance(lm.fit2)[[5]]`, which is zero for all reasonable purposes) indicates a highly statistically significant amount of predictive value is accounted for by the model. This predictive value is no surprise given the moderate R2 value and reasonably large (n = `r nrow(train)`) size of this training sample.
...
Our model formula is `r round(coefficients[[1, 2]], digit=1)`+`r round(coefficients[[2, 2]], digit = 2)`(`r coefficients[[2, 1]]`) `r round(coefficients[[3, 2]], digit =2)`(`r coefficients[[3, 1]]`) `r round(coefficients[[4, 2]], digit =2)`(`r coefficients[[4, 1]]`) `r round(coefficients[[5, 2]], digit =2)`(`r coefficients[[5, 1]]`) `r round(coefficients[[6, 2]], digit =2)`(`r coefficients[[6, 1]]`) `r round(coefficients[[7, 2]], digit =2)`(`r coefficients[[7, 1]]`) `r round(coefficients[[8, 2]], digit=3)` (`r coefficients[[8, 1]]`) `r round(coefficients[[9, 2]], digit=2)`(`r coefficients[[9, 1]]`) `r round(coefficients[[10, 2]], digit=2)`(`r coefficients[[10, 1]]`).  

This implies that for every 1 year increase in Age, we anticipate a drop in the outcome (difference in matching success) of ??.  
...
# Out of data set prediction, predicting probability on test data set with collinear variable of medical_degree removed.  
```{r}
#https://campus.datacamp.com/courses/human-resources-analytics-in-r-predicting-employee-churn/model-validation-hr-interventions-and-roi?ex=1
colnames(test)
...
```
The accuracy of the `limited.vif.model.kitchen.sink` model is `r round(confusion.limited.vif.model.kitchen.sink$overall[[1]], digit=3)`.
...
plot(anova(limited.vif.model.kitchen.sink, test = 'Chisq'), cex=0.9, cex.lab=0.9, cex.axis = 0.7)
```

# Use the Two Models to predict the outcome in the Test Sample
How do the Models perform?  How good is the `limited.vif.model.kitchen.sink` model?  Calculating the Prediction Errors
```{r, include=TRUE}
...
ROC <- pROC::roc(train$Match_Status, step_prob,
    levels=c("1", "0"), direction = "auto")
plot(ROC, col = "red")
pROC::auc(ROC) 
```
Fowards step-wise regression model `step_prob` has an AUC of `r round(pROC::auc(ROC)[[1]], digits=2) ` using these predictors `r step_model$formula[[c(3)]]`.  
...
# Convert the wealth rating to a factor
train$Match_Status <- factor(train$Match_Status, levels = c(0, 1), labels = c("No.Match", "Matched"))
...
t.model <-
  rpart(as.factor(Match_Status) ~.,
        ...
        minsplit = 20)  #Adding this control made the tree much more complicated
...
# Tree visualization
rpart.plot::rpart.plot(t.model,yesno = 2, type = 5, extra = +100, fallen.leaves = TRUE, varlen = 0, faclen = 0, roundint = TRUE, clip.facs = TRUE, shadow.col = "gray", main = "Tree Model of Matching into OBGYN Residency\n(Matched or Unmatched)", box.palette = c("red", "green"))
...
# Tree Visualization
rpart.plot(solution_tree_cv$finalModel, yesno = 2, type = 5, extra = +100, fallen.leaves = TRUE, varlen = 0, faclen = 0, roundint = TRUE, clip.facs = TRUE, shadow.col = "gray", main = "Tree Model of Matching into OBGYN Residency\n(Matched or Unmatched)", box.palette = c("red", "green"))
...
rpart.plot(x = cart, yesno = 2, type = 5, extra = +100, fallen.leaves = TRUE, varlen = 0, faclen = 0, roundint = TRUE, clip.facs = TRUE, shadow.col = "gray", main = "Tree Model of Matching into OBGYN Residency\n(Matched or Unmatched)", box.palette = c("red", "green"))
# Timer off
proc.time() - ptm
cart.model
```
...
```{r generalized logistic regression, include=FALSE, echo=FALSE, cache=TRUE}
DynNom.model.glm  <-
...
```{r sessionInfo, include=FALSE, echo=FALSE, cache=TRUE}
sessionInfo()
```