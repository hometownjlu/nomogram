...
```{r}
all_data <- readr::read_csv(here::here("all_data_output_of_split_correlation.csv"))
train <- readr::read_csv(here::here("train_output_of_split_correlation.csv"))
test <- readr::read_csv(here::here("test_output_of_split_correlation.csv"))
```
...
# class(train)
# sum(is.na(train))
# regressor = stats::lm.fit(formula = formula, data=train, singular.ok = TRUE)
# summary(regressor)
```
...
train1 <- train %>% dplyr::select(-Type_of_medical_school, -US_or_Canadian_Applicant, -Medical_Degree)
...
#Predprob <- stats::predict.glm(object = kitchen.sink, newdata = train1, type = "response")  #not working with knitr or code
