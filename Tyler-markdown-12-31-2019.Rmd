...
all_data$white_non_white <- as.factor(all_data$white_non_white)
all_data$Gender <- as.factor(all_data$Gender)
all_data$Couples_Match <- as.factor(all_data$Couples_Match)
all_data$US_or_Canadian_Applicant <- as.factor(all_data$US_or_Canadian_Applicant)
all_data$Medical_Education_or_Training_Interrupted <- as.factor(all_data$Medical_Education_or_Training_Interrupted)
all_data$Alpha_Omega_Alpha <- as.factor(all_data$Alpha_Omega_Alpha)
all_data$Military_Service_Obligation <- as.factor(all_data$Military_Service_Obligation)
all_data$Visa_Sponsorship_Needed <- as.factor(all_data$Visa_Sponsorship_Needed)
all_data$Medical_Degree <- as.factor(all_data$Medical_Degree)
all_data$Match_Status <- as.factor(all_data$Match_Status)
all_data$Type_of_medical_school <- as.factor(all_data$Type_of_medical_school)
all_data$USMLE_Step_2_CK_Score <- as.numeric(all_data$USMLE_Step_2_CK_Score)
all_data$USMLE_Step_1_Score <- as.numeric(all_data$USMLE_Step_1_Score)
all_data$Medical_Degree <- as.factor(all_data$Medical_Degree)
...
train1 <- train %>% dplyr::select(-Type_of_medical_school, -US_or_Canadian_Applicant, -Medical_Degree, -USMLE_Step_1_Score, -USMLE_Step_2_CK_Score)
...
null_model <- stats::glm(formula = Match_Status ~ 1, data = train, family = "binomial")

# Specify the full model using all of the potential predictors
full_model <- stats::glm(formula = formula, data = train, family = "binomial")
...
train_pca <- caret::preProcess(dplyr::select(all_data, - Match_Status), 
...
h2o::h2o.init()
my_basket.h2o <- h2o::as.h2o(all_data)

# run PCA
my_pca <- h2o::h2o.prcomp(
...
```{r}
levels(train$Match_Status) <- c("0", "1")
...
model_mars
fitted <- stats::predict(model_mars)
fitted
...
cv <- glmnet::cv.glmnet(X, y, alpha=1,nfolds = 10,family="binomial",type.measure = "class")
