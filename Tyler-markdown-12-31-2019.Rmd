...
tm_chi_square_test(train$Medical_Education_Interrupted)#Interrupting medical education is significant
...
# Factor Selection using Forward/Backwards Regression
...
#     Age + white_non_white + Medical_Education_Interrupted + Gender + 
...
# Selected factors:  
# white_non_white + Age + Gender + Couples_Match + US_or_Canadian_Applicant + 
#     Medical_Education_Interrupted + Alpha_Omega_Alpha + Military_Service_Obligation + 
#     USMLE_Step_1_Score + Visa_Sponsorship_Needed + Count_of_Poster_Presentation + 
#     Count_of_Oral_Presentation + Count_of_Articles_Abstracts + 
#     Count_of_Peer_Reviewed_Book_Chapter + Count_of_Other_than_Published + 
#     Count_of_Online_Publications + Visa_Sponsorship_Needed + 
#     Medical_Degree
...
#Needs to be saved.  
```
So how many principal components should we use in the my_basket example? The frank answer is that there is no one best method for determining how many components to use. In this case, differing criteria suggest to retain 5 (scree plot criterion), and 3 (eigenvalue criterion).  
...
Cross-validated RMSE for the 30 different hyperparameter combinations in our grid search. The optimal model retains 17 terms and includes up to 3rd degree interactions.
...
```
The MARS model shows that 6-10 terms were likely to improve accuracy (`model_mars`).  Variable importance shows that US_or_Canadian_applicants, USMLE_Step_1_Score, Age, Medial_Degree, and Visa_Sponsorship_Needed all show significant importance (`varimp_mars`).  
...
legend(0.75, 0.2, legend = c("cv_model3"),
       col = c("black", "blue"), lty = 2:1, cex = 0.6)
tm_print_save(filename = "cv_model3.tiff")
```
...
all_data1 <- all_data %>% select(Age, USMLE_Step_1_Score, Count_of_Articles_Abstracts, Count_of_Poster_Presentation, Count_of_Online_Publications, Count_of_Oral_Presentation, Count_of_Other_than_Published, Count_of_Peer_Reviewed_Book_Chapter, Match_Status)
head(all_data1)

#all_data1 <- all_data %>% as.numeric(as.character())
#  mutate_if(is.factor, is.numeric)
#library(varhandle)
#unfactor(all_data$white_non_white) %>% class()
#as.numeric(levels(all_data$white_non_white)[all_data$white_non_white])

#head(all_data1)
# library(data.table)
# library(mltools)
# customers_1h <- one_hot(as.data.table(all_data))
# all_data1 <- customers_1h
#https://www.datacamp.com/community/tutorials/neural-network-models-r
## Scale data for neural network
#https://www.analyticsvidhya.com/blog/2017/09/creating-visualizing-neural-network-in-r/
#https://uc-r.github.io/ann_regression

# max = apply(all_data1, 2 , max)
# min = apply(all_data1, 2 , min)
# scaled = as.data.frame(scale(all_data1, center = min, scale = max - min))
...
Hab_Data <- all_data1 %>% 
  ...
  mutate(Age = scale01(Age), 
         USMLE_Step_1_Score = scale01(USMLE_Step_1_Score), 
         Count_of_Poster_Presentation = scale01(Count_of_Poster_Presentation), 
         Count_of_Oral_Presentation = scale01(Count_of_Oral_Presentation),
         Count_of_Articles_Abstracts = scale01(Count_of_Articles_Abstracts),
         Count_of_Peer_Reviewed_Book_Chapter = scale01(Count_of_Peer_Reviewed_Book_Chapter),
         Count_of_Other_than_Published = scale01(Count_of_Other_than_Published),
         Count_of_Online_Publications = scale01(Count_of_Online_Publications),
         Match_Status = as.numeric(Match_Status)-1)
Hab_Data %>% glimpse()


set.seed(123)
Hab_NN1 <- neuralnet(Match_Status ~  Age +  USMLE_Step_1_Score + Count_of_Poster_Presentation + Count_of_Oral_Presentation + Count_of_Articles_Abstracts + Count_of_Peer_Reviewed_Book_Chapter + Count_of_Other_than_Published + Count_of_Online_Publications, 
                     data = all_data1, 
                     linear.output = TRUE, 
                     err.fct = 'ce', 
                     likelihood = TRUE)
...
# https://uc-r.github.io/ann_classification
...
Hab_NN2 <- neuralnet(Match_Status ~  Age +  USMLE_Step_1_Score + Count_of_Poster_Presentation + Count_of_Oral_Presentation + Count_of_Articles_Abstracts + Count_of_Peer_Reviewed_Book_Chapter + Count_of_Other_than_Published + Count_of_Online_Publications, , 
                     data = all_data1, 
                     linear.output = FALSE, 
                     err.fct = 'ce', 
                     likelihood = 
...
set.seed(123)
Hab_NN3 <- Hab_NN2 <- neuralnet(Match_Status ~  Age +  USMLE_Step_1_Score + Count_of_Poster_Presentation + Count_of_Oral_Presentation + Count_of_Articles_Abstracts + Count_of_Peer_Reviewed_Book_Chapter + Count_of_Other_than_Published + Count_of_Online_Publications, 
                                data = all_data1, 
                                linear.output = FALSE, 
                                err.fct = 'ce', 
                                likelihood = TRUE, 
...
set.seed(123)
Hab_NN4 <- Hab_NN2 <- neuralnet(Match_Status ~  Age +  USMLE_Step_1_Score + Count_of_Poster_Presentation + Count_of_Oral_Presentation + Count_of_Articles_Abstracts + Count_of_Peer_Reviewed_Book_Chapter + Count_of_Other_than_Published + Count_of_Online_Publications, 
                                data = all_data1, 
                                linear.output = FALSE, 
                                err.fct = 'ce', 
                                likelihood = TRUE, 
...
NNModel <- caret::train(train[,-18], train$Match_Status,
                 method = "nnet",
...
# Create confusion matrix
cmNN <-confusionMatrix(NNPredictions, test$Match_Status)
```
\pagebreak
