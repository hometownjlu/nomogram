...
rf_model <- randomForest::randomForest(formula, data = train)
# Show model error
...
nbayesmodel <- naiveBayes(formula, 
                          data = train)
print(nbayesmodel)
...
```{r}
library(recipes)
xgb_prep <- recipe(formula = formula, data = train) %>%
...
blueprint <- recipe(formula = formula, data = train) %>%
...
Y <- "Match_Status"
X <- setdiff(names(ames_train), Y)  #Why are we using ames_train ???????
```
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, include=TRUE, results="asis"}
table1_all_data <- arsenal::tableby(formula = formula,
                                    data=train, 
