...
```{r}
head(all_data) %>% knitr::kable(format = "pandoc", caption = "Example rows")
```
...
A second way to look at correlation is with pairs of variables plotted.   
...
dev.print(tiff, '/results/tm_nomogram_prep_kitchen_sink.tiff', compression = "lzw",width=2000, height=2000, bg="transparent", res = 200, units = "px" )
dev.off()
```
...
#Accuracy Equation
GLM_Accuracy <- ifelse(GLM_Pred == test$Match_Status1,1,0)
...
confusionMatrix(as.factor(GLM_Pred),as.factor(test$Match_Status1), positive = "1")  ##????

#ROC curve for GLM
library(ROCR)
pred_glm <- prediction(as.numeric(as.character(prob)), as.numeric(as.character(test$Match_Status)))
perf_glm <- performance(pred, "tpr", 'fpr')
...
dev.print(tiff, '~/Dropbox/Nomogram/nomogram/results/tm_variable_importance_caret_matrix.tiff',  width=2000, height=2000, bg="transparent", res = 200,units = "px" )
dev.off()
```
...
head(predNew,10)
confusionMatrix(as.factor(test$Match_Status),as.factor(predNew))
...
head(predNew_cv,10)
confusionMatrix(as.factor(predNew_cv),as.factor(test$Match_Status))
...
##      acc: 0.9994
plot(fit1)
dev.print(tiff, '~/Dropbox/Nomogram/nomogram/results/deep_learning_fit1.tiff', width=2000, height=2000, bg="transparent", res = 200,units = "px" )
dev.off()
```
...
dev.print(tiff, '~/Dropbox/Nomogram/nomogram/results/learning_rate_plot.tiff',  width=2000, height=2000, bg="transparent", res = 200,units = "px" )
dev.off()
