...
The new data set in `all_data`, includes ????? rows and ???? columns, and there are no missing values, as we can see below.  To check in two separate ways.  
```{r, include=TRUE}
...
??? applicants are missing nothing.  
```{r, include=TRUE}
na.pattern(all_data)
```
