...
# Feature Analysis for tidymodels----
```{r}
...
# Plot characteristics for tidymodels ----
```{r}
...
print(paste("Took", round(automl_time, digits = 2), units(automl_time), "to build a automl model."))
```
...
automl_models_h2o <- h2o.automl(
  ...
  #validation_frame = valid_h2o, #grid tuning data set, done automatically by H2O.ai
  leaderboard_frame = test_h2o, #test data set, comment out to do cross-validation
  max_runtime_secs = 0, #originally was 60 secs #0 is no limit to time
  nfolds = 10, #K-fold cross-validation: duplicate the train data into 10 sets.  9 of the 10 are used for training and 1 of the 10 is used for validation.  Different parameters are being evaluate but NOT using a different model.  AUC is generated to measure model effectiveness for each fold and mean is used.
  seed = 1978,
  max_models = 10,
  verbosity = "info",
...
all_data_h2o <- as.h2o(all_data)
all_data_h2o$Match_Status <- as.factor(all_data_h2o$Match_Status)
class(all_data_h2o$Match_Status)
splits <- h2o.splitFrame(all_data_h2o, seed = 1234, ratios = c(0.8, 0.1),
...
# GLM Model
```{r}
#https://github.com/h2oai/h2o-3/blob/master/h2o-r/demos/rdemo.airlines.target_encode.R
## Build GLM
start    <- Sys.time()
...
h2o.confusionMatrix(data.glm, valid=TRUE)
summary(data.glm)
data.glm@model$validation_metrics     ## A more direct way to access the validation 
...
dev.off()
fpr = data.glm@model$training_metrics@metrics$thresholds_and_metric_scores$fpr
tpr = data.glm@model$training_metrics@metrics$thresholds_and_metric_scores$tpr
fpr_val = data.glm@model$validation_metrics@metrics$thresholds_and_metric_scores$fpr
tpr_val = data.glm@model$validation_metrics@metrics$thresholds_and_metric_scores$tpr
plot(fpr,tpr, type='l')
title('Area Under the Curve\n for the GLM Model')
lines(fpr_val,tpr_val,type='l',col='red')
legend("bottomright",c("Train", "Validation"),col=c("black","red"),lty=c(1,1),lwd=c(3,3))

h2o.auc(data.glm,valid=FALSE) # on train    
print(paste("The AUC is:", round(h2o.auc(data.glm,valid=FALSE), digits = 2), "on the train data for the GBM model."))
h2o.auc(data.glm,valid=TRUE)  # on test
print(paste("The AUC is:", round(h2o.auc(data.glm,valid=TRUE), digits = 2), "on the test data for the GBM model."))

data.glm@model$training_metrics@metrics$max_criteria_and_metric_scores

plot(data.glm@model$coefficients, xaxt = "n", xlab = "Coefficients", ylab = "Values")

barplot(data.glm@model$coefficients, main = "Coefficients from Logistic Regression\n of Medical Student Matching Data")

myLabels = c(data.glm@model$x, "Intercept")
plot(data.glm@model$coefficients, xaxt = "n", xlab = "Coefficients", ylab = "Values")
axis(1, at = 1:length(myLabels), labels = myLabels)
abline(h = 0, col = 2, lty = 2)
title("Coefficients from Logistic Regression\n of Medical Students Applying to OBGYN Data")
barplot(data.glm@model$coefficients, main = "Coefficients from Logistic Regression\n of Medical Students Applying to OBGYN Data")
```
...
h2o.confusionMatrix(data.gbm, valid=TRUE)
summary(data.gbm)
data.gbm@model$validation_metrics     ## A more direct way to access the validation 
                                 ##  metrics. Performance metrics depend on 
                                 ##  the type of model being built. With a
                                 ##  multinomial classification, we will primarily
                                 ##  look at the confusion matrix, and overall
                                 ##  accuracy via hit_ratio @ k=1.
dev.off()
fpr = data.gbm@model$training_metrics@metrics$thresholds_and_metric_scores$fpr
tpr = data.gbm@model$training_metrics@metrics$thresholds_and_metric_scores$tpr
fpr_val = data.gbm@model$validation_metrics@metrics$thresholds_and_metric_scores$fpr
tpr_val = data.gbm@model$validation_metrics@metrics$thresholds_and_metric_scores$tpr
plot(fpr,tpr, type='l')
title('Area Under the Curve\n for the GBM Model')
lines(fpr_val,tpr_val,type='l',col='red')
legend("bottomright",c("Train", "Validation"),col=c("black","red"),lty=c(1,1),lwd=c(3,3))

h2o.auc(data.gbm,valid=FALSE) # on train    
print(paste("The AUC is:", round(h2o.auc(data.gbm,valid=FALSE), digits = 2), "on the train data for the GBM model."))
h2o.auc(data.gbm,valid=TRUE)  # on test
print(paste("The AUC is:", round(h2o.auc(data.gbm,valid=TRUE), digits = 2), "on the test data for the GBM model."))

data.gbm@model$training_metrics@metrics$max_criteria_and_metric_scores
```
...
data.drf <- h2o.randomForest(         ## h2o.randomForest function
  training_frame = train,        ## the H2O frame for training
  validation_frame = valid,      ## the H2O frame for validation (not required)
  y = myY,                       ## the predictor columns, by column index
  x = myX,                       ## the target index (what we are predicting)
  model_id = "drf_model",        ## name the model in H2O
...
h2o.confusionMatrix(data.drf, valid=TRUE)
print(data.drf)
summary(data.drf)
data.drf@model$validation_metrics     ## A more direct way to access the validation 
                                 ##  metrics. Performance metrics depend on 
                                 ##  the type of model being built. With a
                                 ##  multinomial classification, we will primarily
                                 ##  look at the confusion matrix, and overall
                                 ##  accuracy via hit_ratio @ k=1.
dev.off()
fpr = data.drf@model$training_metrics@metrics$thresholds_and_metric_scores$fpr
tpr = data.drf@model$training_metrics@metrics$thresholds_and_metric_scores$tpr
fpr_val = data.drf@model$validation_metrics@metrics$thresholds_and_metric_scores$fpr
tpr_val = data.drf@model$validation_metrics@metrics$thresholds_and_metric_scores$tpr
plot(fpr,tpr, type='l')
title('Area Under the Curve\n for the Random Forest Model')
lines(fpr_val,tpr_val,type='l',col='red')
legend("bottomright",c("Train", "Validation"),col=c("black","red"),lty=c(1,1),lwd=c(3,3))

h2o.auc(data.drf,valid=FALSE) # on train    
print(paste("The AUC is:", round(h2o.auc(data.drf,valid=FALSE), digits = 2), "on the train data for the Random Forest model."))
h2o.auc(data.drf,valid=TRUE)  # on test
print(paste("The AUC is:", round(h2o.auc(data.drf,valid=TRUE), digits = 2), "on the test data for the Random Forest model."))

data.drf@model$training_metrics@metrics$max_criteria_and_metric_scores
```

# Build Deep Learning Model
```{r}
start   <- Sys.time()
data.dl <- h2o.deeplearning(y = myY, 
                            x = myX, 
                            training_frame = data.train, 
                            validation_frame = data.test, 
                            hidden=c(10, 10),
...
summary(data.dl)
data.dl@model$validation_metrics     ## A more direct way to access the validation 
                                 ##  metrics. Performance metrics depend on 
                                 ##  the type of model being built. With a
                                 ##  multinomial classification, we will primarily
                                 ##  look at the confusion matrix, and overall
...
h2o.confusionMatrix(data.dl, valid=TRUE)
summary(data.dl)
data.dl@model$validation_metrics     ## A more direct way to access the validation 
                                 ##  metrics. Performance metrics depend on 
                                 ##  the type of model being built. With a
                                 ...
                                 ##  accuracy via hit_ratio @ k=1.
dev.off()
fpr = data.dl@model$training_metrics@metrics$thresholds_and_metric_scores$fpr
tpr = data.dl@model$training_metrics@metrics$thresholds_and_metric_scores$tpr
fpr_val = data.dl@model$validation_metrics@metrics$thresholds_and_metric_scores$fpr
tpr_val = data.dl@model$validation_metrics@metrics$thresholds_and_metric_scores$tpr
plot(fpr,tpr, type='l')
title('Area Under the Curve\n for the Deep Learning Model')
lines(fpr_val,tpr_val,type='l',col='red')
legend("bottomright",c("Train", "Validation"),col=c("black","red"),lty=c(1,1),lwd=c(3,3))

h2o.auc(data.dl,valid=FALSE) # on train    
print(paste("The AUC is:", round(h2o.auc(data.dl,valid=FALSE), digits = 2), "on the train data for the Deep Learning model."))
h2o.auc(data.dl,valid=TRUE)  # on test
print(paste("The AUC is:", round(h2o.auc(data.dl,valid=TRUE), digits = 2), "on the test data for the Deep Learning model."))

data.dl@model$training_metrics@metrics$max_criteria_and_metric_scores
```
...
data.glm@model$standardized_coefficient_magnitudes %>% as_tibble()
print("GBM: Variable Importance")
data.gbm@model$variable_importances %>% as_tibble()
print("Random Forest: Variable Importance")
data.drf@model$variable_importances %>% as_tibble()
print("Deep Learning: Variable Importance")
data.dl@model$variable_importances %>% as_tibble()
```
...
a <- h2o.hit_ratio_table(data.glm,valid = F, train = F, xval = FALSE)#[1,2] 
class(a)
describe(a)
## validation set accuracy
mean(finalRf_predictions$predict==test_h2o$Match_Status)  ## test set accuracy
