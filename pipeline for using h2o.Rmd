...
summary(data.gbm) #Original
summary(gbm2)
h2o.hit_ratio_table(data.gbm,valid = T)[1,2]    ## review the first model's accuracy
#h2o.hit_ratio_table(gbm2,train = FALSE, valid = FALSE)[2,1]  ## review the new model's accuracy
###############################################################################
```
...
summary(data.drf)
data.drf@model$validation_metrics 
```
...
myX <- setdiff(colnames(train), c("Match_Status"))
...
default_gbm <- h2o.gbm(x = myX, y = "Match_Status",
...
print("Perform Leave One Out Target Encoding on US_or_Canadian_Applicant")

##???
#te_cols <- list(c("US_or_Canadian_Applicant")) #onlu works with one variable at a time right now.  Could try: x = list(c("job"), c("job", "marital")), from documentation.    
te_cols <- list(c("USMLE_Step_1_Score"), c("USMLE_Step_1_Score", "Age"), c("USMLE_Step_1_Score", "Age", "Medical_Degree")) 
...
encoding_map <- h2o.target_encode_create(full_train, te_cols, "Match_Status")
head(encoding_map)

# Apply Leave One Out Encoding Map on Training, Validation, Testing Data
loo_train <- h2o.target_encode_apply(loo_train, x = te_cols,  y = "Match_Status", encoding_map, holdout_type = "LeaveOneOut", seed = 1234)
loo_valid <- h2o.target_encode_apply(loo_valid, x = te_cols, y = "Match_Status", encoding_map, holdout_type = "None", noise_level = 0)
loo_test <- h2o.target_encode_apply(loo_test, x = te_cols, y = "Match_Status", encoding_map, holdout_type = "None", noise_level = 0)

print("Run GBM with Leave One Out Target Encoding")
myX <- setdiff(colnames(loo_test), c(te_cols, "Match_Status", "Match_Status_REC"))
loo_gbm <- h2o.gbm(x = myX, y = "Match_Status",
...
print("Perform Target Encoding on USMLE_Step_1_Score, Age, and Medical_Degree with Cross Validation Holdout")

te_cols <- list(c("USMLE_Step_1_Score"), c("USMLE_Step_1_Score", "Age"), c("USMLE_Step_1_Score", "Age", "Medical_Degree")) #onlu works with one variable at a time right now.  Could try: x = list(c("USMLE_Step_1_Score"), c("job", "marital")), from documentation.   
...
# Create Encoding Map
encoding_map <- h2o.target_encode_create(cv_train, te_cols, "Match_Status", "fold")

# Apply Encoding Map on Training, Validation, Testing Data
cv_train <- h2o.target_encode_apply(cv_train, x = te_cols,  y = "Match_Status", encoding_map, holdout_type = "KFold", fold_column = "fold", seed = 1234)
cv_valid <- h2o.target_encode_apply(cv_valid, x = te_cols, y = "Match_Status", encoding_map, holdout_type = "None", fold_column = "fold", noise_level = 0)
cv_test <- h2o.target_encode_apply(cv_test, x = te_cols, y = "Match_Status", encoding_map, holdout_type = "None", fold_column = "fold", noise_level = 0)

print("Run GBM with Cross Validation Target Encoding")
cvte_gbm <- h2o.gbm(x = myX, y = "Match_Status",
...
# Create Leave One Out Encoding Map
encoding_map <- h2o.target_encode_create(te_holdout, te_cols, "Match_Status")

# Apply Encoding Map on Training, Validation, and Testing Data
holdout_train <- h2o.target_encode_apply(holdout_train, x = te_cols, y = "Match_Status", encoding_map, holdout_type = "None", noise_level = 0)
