...
set.seed(0)
train_corrleation <- train %>% select(Age, USMLE_Step_1_Score, Count_of_Poster_Presentation, Count_of_Oral_Presentation, Count_of_Peer_Reviewed_Articles_Abstracts, Count_of_Peer_Reviewed_Book_Chapter, Count_of_Peer_Reviewed_Other_than_Published, Count_of_Online_Publications) %>%
...
```{r, echo=TRUE}
lm.fit2 <- lm(Match_Status ~ (white_non_white + Age + Gender +  Couples_Match + US_or_Canadian_Applicant +  Medical_Education_Interrupted + Alpha_Omega_Alpha +  Military_Service_Obligation + USMLE_Step_1_Score + Count_of_Poster_Presentation +  Count_of_Oral_Presentation + Count_of_Peer_Reviewed_Articles_Abstracts + Count_of_Peer_Reviewed_Book_Chapter + Count_of_Peer_Reviewed_Other_than_Published + Count_of_Online_Publications + Visa_Sponsorship_Needed + Medical_Degree)^2, data = all_data)
```
...
# WOE binning 
# Confirming target variable is numeric and 0 or 1
test$Match_Status <- as.numeric(train$Match_Status)
table(test$Match_Status)
test <- as.data.frame(test)

# Bin all variables of the data frame (apart from the target variable)
# with default parameter settings
binning <- woe.binning(test, 'Match_Status', c('Age','USMLE_Step_1_Score', "Count_of_Poster_Presentation", "Count_of_Oral_Presentation", "Count_of_Articles_Abstracts", "Count_of_Peer_Reviewed_Book_Chapter", "Count_of_Other_than_Published", "Count_of_Online_Publications"))
