...
ggplot(all[!is.na(all_data$Match_Status),], aes(x = Survived, fill = Survived)) +
  geom_bar(stat='count') +
...
train_correlation <- 
  ...
  #mutate_all(as.integer) %>%  #I turned this off and it might cause problems later.  
...
# Fitting and Summarizing the Kitchen Sink Model
...
caret::confusionMatrix(solution_rf, test$Match_Status)
```
...
## The support vector machine model was fit. Cross validation identified a cost C = 100 using a linear kernel and a sigma =  ???  using ??? support vectors.
...
caret::confusionMatrix(solution_svm, test$Match_Status)
```
