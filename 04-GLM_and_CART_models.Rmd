...
all_data <- readr::read_csv("~/Dropbox/Nomogram/nomogram/all_data_output_of_split_correlation.csv") %>%
  dplyr::select("ACLS", "Age", "Alpha_Omega_Alpha",  "BLS", "Citizenship",  "Count_of_Peer_Reviewed_Journal_Articles_Abstracts", "Count_of_Poster_Presentation", "Couples_Match", "Gender", "Medical_Education_or_Training_Interrupted", "NIH_dollars", "PALS",  "Type_of_medical_school", "US_or_Canadian_Applicant", "USMLE_Step_1_Score", "USMLE_Step_2_CK_Score", "Visa_Sponsorship_Needed", "white_non_white", "Match_Status")
...
# Final selection of variables from Boruta
```{r, include=TRUE}
boruta_drivers <- c(
                                                                     "Age",
                                                       "Alpha_Omega_Alpha",
...
# 1) Model: Caret for Fitting Generalized Linear Models
...
train <- train %>%
dplyr::mutate(Match_Status = dplyr::recode(Match_Status, `1` = "Match", `0` = "Did.not.match"))
...
\pagebreak
# 2) Draw a Nomogram Representing a Regression Fit
...
```{r, include = TRUE, , fig.width = 8, dpi = 1200}
...
dd <- datadist(test)
options(datadist = "dd")
lrm.version.of.cv_model3 <- 
  rms::lrm(formula = Match_Status ~ Age + Alpha_Omega_Alpha + ACLS + BLS + Citizenship + Count_of_Peer_Reviewed_Journal_Articles_Abstracts + Count_of_Poster_Presentation + Couples_Match + Gender + Medical_Education_or_Training_Interrupted + NIH_dollars + PALS + Type_of_medical_school  + US_or_Canadian_Applicant  + USMLE_Step_1_Score  + USMLE_Step_2_CK_Score + Visa_Sponsorship_Needed + white_non_white, 
      ...
      y = T,
      maxit=10000)
#singular information matrix in lrm.fit (rank= 16 ).  Offending variable(s):
#USMLE_Step_1_Score US_or_Canadian_Applicant=US senior 
#Only thing that fixed this was decreasing the singularity criteria set by tol, tol = 1e-9 works
...
nomo <- rms::nomogram(lrm.version.of.cv_model3,  #requires an lrm model from rms package
...
```{r NOMOGRAM2,  fig.width=7, fig.asp=1}
plot(anova(lrm.version.of.cv_model3))
```
From the ANOVA plot, it looks like a model with Step 1, Step 2 scores, and citizenship might be sufficient.  
...
tm_print_save("train_lrm_with_lrm_version_of_cv_model3.tiff")
```
...
```{r}
#https://rpubs.com/TELOVE/project1_demo_2019-432
plot(rms::calibrate(fit = lrm.version.of.cv_model3,
...
tm_print_save("train_lrm_with_lrm_version_of_cv_model3_calibration_plot.tiff")
```
`cv_model3` appears to overpredict a bit, especially at the lower and the higher predicted values.

## Validation of `lrm.version.of.cv_model3`, with backwards elimination
What if we try a validation step, with a stepwise backwards elimination approach?
```{r}
set.seed(432)
a <- rms::validate(lrm.version.of.cv_model3, bw = TRUE, B = 1000)
a
#https://rpubs.com/TELOVE/project1_demo_2019-432
```
...
which is a cross-validated estimate of the value of the C statistic we might reasonably expect using this model. Compare this to the nominal C = 0.86 (`r lrm.version.of.cv_model3[[3]]`) shown in the initial `lrm.version.of.cv_model3` summary

So, it looks like this model is pretty seriously overfitting the data. The next model to try involves (see "Factors in Final Model" from rms::validate(lrm....)) only:  
`Age`, 
`Citizenship`, 
`Medical_Education_or_Training_Interrupted`, 
`Type_of_medical_school`, 
`USMLE_Step_1_Score`, 
`USMLE_Step_2_CK_Score`, and 
`white_non_white.`
...
dd <- datadist(train)
options(datadist = "dd")
m.2 <- lrm(Match_Status ~ Age + Citizenship + Medical_Education_or_Training_Interrupted + Type_of_medical_school + USMLE_Step_1_Score + USMLE_Step_2_CK_Score + white_non_white,
           data = train, x = T, y = T)
m.2
```
...
```{r}
plot(nomogram(m.2))
tm_print_save("model_2_nomogram.tiff")
```

## Making predictions with m.2
Let’s make a mean prediction for two subjects, each with `USMLE_Step_1_Score` = 230 but one with `Citizenship` = “US Citizen” and the other with `Citizenship` = “Not A Citizen”, using a 90% confidence interval.

```{r}
rms::Predict(m.2, Citizenship = c("US_Citizen", "Not_A_Citizen"), USMLE_Step_1_Score = 230, conf.int = 0.90, fun = plogis)
```

##Plotting Model 2 Predictions
Here is a plot of the predictions across the two levels of `Citizenship` for patients with USMLE_Step_1_Score ranging from 220 to 280.

```{r}
ggplot(rms::Predict(m.2, USMLE_Step_1_Score = 220:280, 
               Citizenship = c("US_Citizen", "Not_A_Citizen"), fun=plogis)) +
    theme_bw() +
    labs(x = "USMLE Step 1 Score of Applicants to OBGYN",
...
```{r}
model.2 <- stats::glm(Match_Status ~ Age + Citizenship + Medical_Education_or_Training_Interrupted + Type_of_medical_school + USMLE_Step_1_Score + USMLE_Step_2_CK_Score + white_non_white, 
                      data = train, 
               family = binomial(link = logit))
```
...
prob <- stats::predict(model.2, data = train, type="response")
pred <- ROCR::prediction(prob, train$Match_Status)
# rest of this doesn't need much adjustment except for titles
...
ggplot2::ggplot(roc.data, aes(x=fpr, ymin=0, ymax=tpr)) +
    ...
    labs(title = paste0("ROC Curve w/ AUC=", auc),
         subtitle = "Model 2 for train data")
```
...
# 3)  Model: A CART model was fit using the rpart package. 
