...
#singular information matrix in lrm.fit (rank= 18 ).  Offending variable(s):
#Type_of_medical_school=Osteopathic School 
        US_or_Canadian_Applicant + Visa_Sponsorship_Needed + white_non_white, 
...
which is a cross-validated estimate of the value of the C statistic we might reasonably expect using this model. Compare this to the nominal C = 0.86 (`r m.1[[3]]`) shown in the initial `m.1` summary


# 2) Model 2 (m.2): a smaller model with promising predictors

We’ll start with a model motivated by the Spearman ρ2 plot developed above, and repeated below.
```{r}
train <- readr::read_csv("~/Dropbox/Nomogram/nomogram/train_output_of_split_correlation.csv") %>%
  dplyr::select("Age", "Alpha_Omega_Alpha", "BLS","Citizenship", "Gender", "Medical_Degree", "Medical_Education_or_Training_Interrupted",  "US_or_Canadian_Applicant",  #"USMLE_Step_1_Score", 
                #"USMLE_Step_2_CK_Score", 
...
dd <- datadist(train)
options(datadist = "dd")
m.2 <- lrm(Match_Status ~ Age +  Alpha_Omega_Alpha + BLS +Citizenship + Gender + Medical_Degree + Medical_Education_or_Training_Interrupted + US_or_Canadian_Applicant +
             #USMLE_Step_1_Score + 
             #USMLE_Step_2_CK_Score + 
             white_non_white,
           data = train, x = T, y = T)
...
##Nomogram for m.2
```{r, include = TRUE, , fig.width = 12, fig.height = 12, dpi = 1200}
#https://rpubs.com/TELOVE/project1_demo_2019-432
nomo.m.2 <- rms::nomogram(m.2,  #requires an lrm model from rms package
                lp.at = seq(-3,4,by=0.5),
                fun = plogis,
                fun.at = c(0.001, 0.01, 0.05, seq(0.2, 0.8, by = 0.2), 0.95, 0.99, 0.999),
                funlabel = "Chance of Matching in OBGYN",
...
plot(nomo.m.2)
tm_print_save("nomo_model_2_nomogram.tiff")
```

## Making predictions with m.2
Let’s make a mean prediction for two subjects, each with `Age` = 26 but one with `Citizenship` = “US Citizen” and the other with `Citizenship` = “Not A Citizen”, using a 90% confidence interval.

```{r}
rms::Predict(m.2, Citizenship = c("US_Citizen", "Not_A_Citizen"), Age = 26, conf.int = 0.90, fun = plogis)
```
...
model.2 <- stats::glm(Match_Status ~ Age +  Alpha_Omega_Alpha + BLS +Citizenship + Gender + Medical_Degree + Medical_Education_or_Training_Interrupted + US_or_Canadian_Applicant +
             #USMLE_Step_1_Score + 
             #USMLE_Step_2_CK_Score + 
             white_non_white,
                      data = train, 
...
pred <- ROCR::prediction(predictions = prob,   #Contains the predictions
                         labels = train$Match_Status)  #contains the truth
...
# Model without Step_2
```{r}
train <- readr::read_csv("~/Dropbox/Nomogram/nomogram/train_output_of_split_correlation.csv") %>%
  dplyr::select("Age", "Citizenship",  "Medical_Education_or_Training_Interrupted",  "Type_of_medical_school",  #"USMLE_Step_1_Score", 
                #"USMLE_Step_2_CK_Score", 
                "white_non_white", "Match_Status")

test <- readr::read_csv("~/Dropbox/Nomogram/nomogram/train_output_of_split_correlation.csv") %>%
  dplyr::select("Age", "Citizenship",  "Medical_Education_or_Training_Interrupted",  "Type_of_medical_school",  #"USMLE_Step_1_Score", 
                #"USMLE_Step_2_CK_Score", 
                "white_non_white", "Match_Status")
```
...
dd <- datadist(train)
options(datadist = "dd")
m.2 <- lrm(Match_Status ~ Age + Citizenship + Medical_Education_or_Training_Interrupted + Type_of_medical_school + 
             #USMLE_Step_1_Score + 
             #USMLE_Step_2_CK_Score 
             + white_non_white,
           data = train, x = T, y = T)
...
anova(m.2)
plot(anova(m.2))
tm_print_save("model_2_fewers_variables_plot_no_step_2.tiff")
```
...
summary(m.2)
plot(summary(m.2))
tm_print_save("model_2_fewers_odds_ratio_plot_no_step_2.tiff")
```

The calibration plot does not look much different.  
```{r}
plot(rms::calibrate(m.2))
tm_print_save("model_2_fewers_calibration_plot_no_step_2.tiff")
```

##Nomogram for m.2
```{r, include = TRUE, , fig.width = 12, fig.height = 12, dpi = 1200}
#https://rpubs.com/TELOVE/project1_demo_2019-432
nomo.m.2.no.step.2 <- rms::nomogram(m.2,  #requires an lrm model from rms package
                lp.at = seq(-3,4,by=0.5),
                fun = plogis,
                fun.at = c(0.001, 0.01, 0.05, seq(0.2, 0.8, by = 0.2), 0.95, 0.99, 0.999),
                funlabel = "Chance of Matching in OBGYN",
...
plot(nomo.m.2.no.step.2)
tm_print_save("nomo_model_2_nomogram_no_step_2.tiff")
```
...
The biggest factor is the type of medical school attended.  The Decision Tree rules run from the top to the bottom. If you look at the top of the tree, the 1st rule is “International medical school”. “Yes” goes to the left hand side, and “No” goes to the right. If you look at the right box at the 2nd level, you can tell that the 76% of applicants were not IMGs, and your match rate is 90% (0.9). If you are an international medical school graduate, chances are only 54%.
