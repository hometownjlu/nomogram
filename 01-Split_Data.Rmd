...
```{r}
knitr::include_graphics(here::here("images/machine-learning-splitting-datasets-3-638.jpg"))
```

```{r, include=FALSE}
all_data <- readr::read_csv(here::here("all_data_end_of_EDA.csv"))
```
...
train <- data_split %>% rsample::training() %>% 
  dplyr::select(-contains("USMLE")) %>% 
  dplyr::select(-Medical_School_of_Graduation) %>%
  dplyr::glimpse()  # Extract the training dataframe
...
test <- data_split %>% rsample::testing() %>% 
  dplyr::select(-contains("USMLE")) %>% 
    dplyr::select(-Medical_School_of_Graduation) %>%
  dplyr::glimpse() # Extra
...
```{r, include=FALSE}
readr::write_csv(all_data, (here::here("all_data_output_of_split_correlation.csv")))
readr::write_csv(train, (here::here("train_output_of_split_correlation.csv")))
readr::write_csv(test, (here::here("test_output_of_split_correlation.csv")))
```
