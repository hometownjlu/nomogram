...
ggpairs(data=all_data_ggally, progress=TRUE, columns = c("Match_Status" , "US_or_Canadian_Applicant" , "Couples_Match", "Visa_Sponsorship_Needed", "Count_of_Poster_Presentation"))
```
...
We load the caret package to dummify all factor variables as the cor function only accepts numerical values:
```{r, echo=FALSE, results="asis"}
#https://youtu.be/igPQ-pI8Bjo?t=269
dmy <- dummyVars(" ~ .", data = train)
adultsTrsf <- data.frame(predict(dmy, newdata = train))
...
selectedSub <- subset(corList, (abs(cor) > 0.2 & j == 'Match_Status'))
print(selectedSub)

bestSub <- as.character(selectedSub$i)
bestSub

knitr::kable(bestSub, caption = "Highly Correlated Factors for Matching into OBGYN")
```

The pairs plot, and in particular the last Match_Status column, tell us a lot about our data set. Being a US or Canadian applicant is the most positively correlated with Matching_Status. Not being a US citizen and Age are the most negatively correlated.
```{r, echo=FALSE}
correlation_plot <- pairs.panels(adultsTrsf[c(bestSub, 'Match_Status')])
```
