...
```
The accuracy of the supported vector machine model is `r round(caret_svm$results[[2]], digits=2)`.  
...
a <- caret::postResample(pred = train.cart, obs = as.factor(train$Match_Status))
...
Look for skewed data to determine what needs to be scaled and centered prior to modeling with tidymodels.  Not necessary for h2o because h2o does all the centering and scaling on its own.  
```{r}
...
train_tbl %>%
  get_cor(target = Match_Status_Matched, fct_reorder = TRUE, fct_rev = TRUE)

train_tbl %>%
  #select(Match_Status_Matched, contains("Count_of_")) %>%
  plot_cor(target = Match_Status_Matched, fct_reorder = TRUE, fct_rev = FALSE)
