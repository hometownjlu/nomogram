...
The list of applicants and their match status was a compilation from multiple sources.  Ultimately the primary sources listing residents are ACGME and ABOG.  Neither organization will likely share their rosters due to privacy concerns.  Therefore we had over 95% agreement between these three sources:  

1)  The data source to be used for the project is the Electronic Residency Application Service data that was in discrete fields in the application.  No hand-searching of data was done. The data was exported from the ERAS Program Director Work Station under the Archives menu.The data is collected by the University of Colorado OBGYN residency that has both a categorical and a preliminary position.  Medical students who applied to the preliminary position were considered to be unmatched. The data set of years 2015, 2016, 2017, and 2018 applicants to the University of Colorado OBGYN residency. 2019 data includes SOAP participation.   https://apps.aamc.org/account/#/login?gotoUrl=http:%2F%2Fapps.aamc.org%2Feras-pdws-web%2F&allowInternal=false 

2)  A second set of data is the NPI database.  Every physician needs an NPI number to practice and a specialty designation called the taxonomy code.  Residents are required to have this as well.  NPI data distinguishes between specialties so can tell if people applied to OBGYN but failed to match and scrambled into another field.  
https://npiregistry.cms.hhs.gov 

3)  Lasty, medical schools often publish a list of where their students will be matriculating to.  This is public information and often posted online.  https://www.dropbox.com/sh/ugtgclo02zcv48n/AAC7QWUUSsEdP6gcW6LRY74ta?dl=0 

I would like to create a Venn diagram dmeonstrating the overlap in the data sources.  
...
10. `all_data$USMLE_Step_1_Score` - I did not use Step 1 score because it will be pass/fail starting in 2020, numerical variable
...
recipe_simple <- function(dataset) {
  ...
  recipes::step_string2factor(all_nominal(), -all_outcomes()) %>%
    ...
    add_role(white_non_white, Age, Gender, Couples_Match, US_or_Canadian_Applicant, Medical_Education_Interrupted, Alpha_Omega_Alpha, Military_Service_Obligation, #USMLE_Step_1_Score, 
...
library(magrittr)
all_data <- all_data %>%
  dplyr::select(-Applicant_Name) #MAY KEEP THIS IN.  
```
...
library(DT)
DT::datatable(all_data, options = list(pageLength = 50))
```
...
1. All factors had two levels except for Alpha_Omega_Alpha had three levels.  The target variable is `all_data$Match_Status`.  
2. The remainder of the variables were integers. 
...
#desc_groups(data=all_data, group_var="Match_Status")  
df_status_output
...
```{r funModeling4, echo=TRUE, message=TRUE, warning=TRUE, include=TRUE, out.width="75%"}
...
```{r, echo=TRUE, warning=FALSE, message=TRUE, include=TRUE, results="asis"}
...
```{r}
readr::write_csv(all_data, (here::here("all_data_end_of_EDA.csv")))
file.exists(here::here("all_data_end_of_EDA.csv"))
```
