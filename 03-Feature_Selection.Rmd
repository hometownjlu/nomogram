---
title: "03-Feature_Selection: A Model to Predict Chances of Matching into Obstetrics and Gynecology Residency"
...
smart: yes
---
# Read in the original `all_data`, `train`, and `test` while removing `USMLE_Step_1_Score` and `USMLE_Step_2_CK_Score`.  
...
```{r, include=FALSE, echo=FALSE}
library(magrittr)
all_data <- readr::read_csv(here::here("all_data_output_of_split_correlation.csv")) %>% dplyr::select( -contains("USMLE"))

train <- readr::read_csv(here::here("train_output_of_split_correlation.csv")) %>% dplyr::select( -contains("USMLE"))

test <- readr::read_csv(here::here("test_output_of_split_correlation.csv")) %>% dplyr::select( -contains("USMLE"))
...
`Malpractice_Cases_Pending`, Removed because this is due to the fact that all people had 0 cases/zero variance variable.  All other variables that are commented out were excluded from Boruta.  `Applicant_Name` was removed for privacy concerns.  
...
formula
```
I spent a lot of time getting the feature selection right because of the possible complaints of discrimination when saying that gender and age and other stuff matters.  This is specifically to address Dr. Santoro's complaints while creating a parisimonious and fast model.  
...
```{r}
knitr::include_graphics("images/Boruta_random_forest.png")
```

In short, Boruta runs the Random Forest model building many times (the default is 20) and therefore you can see the importance scores as distribution with Boxplot, not as a single value with Bar chart.
...
```{r}
knitr::include_graphics("images/shadow_variables.png")
```


```{r}
knitr::include_graphics("images/shadow_variables_best_scoring.png")
```
...
```{r, label = BORUTA, include=FALSE, echo=FALSE, cache=TRUE, warning=FALSE}
...
```{r, include=FALSE, echo=FALSE, cache=FALSE, warning=FALSE}
#names(boruta_output)
...
```{r}
knitr::include_graphics("images/Boruta RandomForest_Importance.png")
```
The green color indicates that these variables are statistically confirmed that they are better than the ‘Shadow’ variables hence they are useful to predict the target variable.

On the other hand, the gray color indicates that these variables are statistically confirmed that they are worse than the ‘Shadow’ variables thence they are useless to predict the target variable.

Now, here is yellow. These variables mean that they are better than ‘Shadow’ variables sometimes but worse sometimes, and the test can’t conclude either way.
...
# train <- readr::read_csv("~/Dropbox/Nomogram/nomogram/train_output_of_split_correlation.csv") %>% dplyr::select(-USMLE_Step_1_Score, -USMLE_Step_2_CK_Score)
# test <- readr::read_csv("~/Dropbox/Nomogram/nomogram/test_output_of_split_correlation.csv") %>% dplyr::select(-USMLE_Step_1_Score, -USMLE_Step_2_CK_Score)
# 
# train <- train %>%
# dplyr::mutate(Match_Status = dplyr::recode(Match_Status, `Did Not Match` = "Did.Not.Match", Match = "Matched"))
# 
...
```{r}
#Forward regression
#knitr::kable(as.data.frame(forward_drivers), caption = "Variables chosen from forward regression")
```

## Variables selected by backward regression
```{r}
#Backward regression
#knitr::kable(as.data.frame(backward_drivers), caption = "Variables chosen from backwards regression")
```
